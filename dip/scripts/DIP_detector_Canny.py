...
class PipelineCanny:
    ...
    def ImageProcessor(self):
        ...
        # object detection
        print('running')
        canny_bbox, pose, triangle = detect_object(self.imageLeft, self.imageRight)
        n_, m_, _ = self.imageLeft.shape
        
        # rospy.loginfo(BBox)
        if canny_bbox == [-1,-1,-1,-1]:
            ...
            if canny_bbox == [-1,-1,-1,-1]:
            ...
            elif canny_bbox != [-1,-1,-1,-1]:
                rospy.loginfo("New Object found")
                top_left_x, top_left_y, width, height = canny_bbox
                ...
                self.mutex.release()
                self.bbox_pub.publish(mobs)
                self.temp_BBox = canny_bbox
                
                if self.publish_image:
                    rospy.loginfo ("Object detected >> Bbox: {0}".format(canny_bbox))
                    for pts in pose:
                        cv2.circle(self.imageLeft, (int(pts[0]), int(pts[1])), 3, (0,255,0), -1)
                    cv2.rectangle(self.imageLeft, (canny_bbox[0], canny_bbox[1]), (canny_bbox[0] + canny_bbox[2], canny_bbox[1] + canny_bbox[3]), (36,255,12), 2)
